...
def index():
    ...
    usdt = c.execute('SELECT date,cumulative_usd FROM income WHERE type = ? ORDER BY timestamp LIMIT 30',("USDT",)).fetchall()
    btc = c.execute('SELECT date,cumulative_usd FROM income WHERE type = ? ORDER BY timestamp LIMIT 30',("BTC",)).fetchall()
